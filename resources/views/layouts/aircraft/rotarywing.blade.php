...
<div class="wrapper">
  <div class="mySwiper d-flex justify-content-center">
	...
	@foreach($rotary as $rw)
	<div class="main swiper-slide" id="{{$rw -> i_ptdi_ac}}">
            <div class="header-tittle-box">
              <div class="main-wrapper container d-flex justify-content-center">
              @php $titlepertama = implode(" ", array_slice(explode(" ", $rw->n_ptdi_ac), 0, "1"));@endphp 
                <h1 class="main-title-rotary text-uppercase">{{$titlepertama}}</h1>
              </div>
            </div>
            ...
            <div class="main-content">
              <div class="main-content__title text-uppercase">{{$rw -> n_ptdi_ac}}</div>
              <div class="main-content__subtitle">{!! $rw -> e_ptdi_acreviewen !!}</div>
              <div class="more-menu">
                <a class="btn btn-warning btn-sm rounded-pill" href="/detail-{{$rw->i_ptdi_ac}}-{{strtolower(Str::replace(array(' ','/'), '-', $rw->n_ptdi_ac))}}">Detail</a>
              </div>
            </div>
  </div>
